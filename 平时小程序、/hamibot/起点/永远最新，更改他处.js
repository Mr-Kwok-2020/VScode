...
function qiandao() {
    ...
    if (desc("明天再来").exists() || desc("明日再来抽奖").exists()) {
        ...
        do {
            ...
            while (!(desc("抽 奖").exists() || desc("看视频抽奖机会+1").exists()) && !(desc("明天再来").exists() || desc("明日再来抽奖").exists())) {
                sleep(500)
            }
            if (desc("抽 奖").exists()) {
                //点击抽奖
            ...
            } else if (desc("看视频抽奖机会+1").exists()) {
                //看视频
                while (clickParentIfClickable(desc("看视频抽奖机会+1").findOne(1000)) == null) {
                    swipe(centerX, centerY, centerX, centerY - 100, 100)
                }
                waitad()
            }
