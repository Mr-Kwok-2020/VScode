...
# with ThreadPoolExecutor(max_workers=20) as executor:  # 设定线程数量
#     executor.map(process_iteration, range(0, 5, 1))

from concurrent.futures import ThreadPoolExecutor, as_completed

# 使用 ThreadPoolExecutor 并行执行
with ThreadPoolExecutor(max_workers=4) as executor:  # 设定线程数量
    # 提交任务到线程池
    futures = [executor.submit(process_iteration, ghgjh) for ghgjh in range(0, 5, 1)]
    
    # 确保每个任务完成后才继续
    for future in as_completed(futures):
        future.result()  # 结果会被自动处理


print("All random numbers:", all_rand_num[:10])  # 示例：打印前10个结果
print("All pred_y1_mse:", all_pred_y1_mse[:10])
print("All pred_y2_mse:", all_pred_y2_mse[:10])
